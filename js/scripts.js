...
function aumentarValor() {
    ...
    if (valoresDesejados.length > 0) {
        ...
        if (valoresDesejados.length === 0) {
            // Update the popup value to match current balance
            const valorSpanPopup = document.querySelector('#popupL #valor-saque');
            if (valorSpanPopup) {
                valorSpanPopup.textContent = `R$ ${valor.toFixed(2)}`;
            }
            showPopupL();
        }
    }
...
function verificarValor() {
   // Logic removed as per new requirement to only show popup at the end
}
